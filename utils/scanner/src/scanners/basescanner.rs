    ...
    fn in_scan_action(&self, cur_entry: &str) -> Result<(Vec<String>, Vec<String>)> {
        ...
        if metadata.is_dir() {
            for entry in WalkDir::new(cur_entry).into_iter().filter_map(Result::ok) {
                ...
                trace!("{} found!", entry_path);

                src_paths.push(entry_path.to_string());
                des_paths.push(self.des_path.to_string() + &entry_path.replace(parent_entry, "/"));
            }
        } else {
...
mod tests {
    ...
    fn scan_dir_works() {
        ...
        assert_eq!(src_paths, vec![sfolder, sfile]);
        ...
        assert_eq!(des_paths, vec![dfolder, dfile]);
    }
